...
final class ProductViewFactory implements ProductViewFactoryInterface
{
    ...
    {
        // TODO Check customer zone
        /** @var ProductVariantInterface $mainProductVariant */
        $mainProductVariant = $product->getVariants()->first();
        $mainProductVariantTaxCategory = $mainProductVariant->getTaxCategory();
        $currentTaxRateAmount = 0;
        if ($mainProductVariantTaxCategory !== null) {
            /** @var TaxRateInterface $mainProductVariantMainTaxRate */
            $mainProductVariantMainTaxRate = $mainProductVariantTaxCategory->getRates()->first();
            $currentTaxRateAmount = $mainProductVariantMainTaxRate->getAmount() * 100;
        }

        /** @var ProductView $productView */
        $productView = new $this->productViewClass();
        $productView->code = $product->getCode();
        $productView->taxRateAmount = $currentTaxRateAmount;
