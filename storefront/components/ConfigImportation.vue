<template>
  <div>
    ...
    <v-row dense>
      <v-col cols="12" sm="8" class="body-2 justify-center">
        <CtBtn
          ...
          >Configuración Importaciones</CtBtn
        >
    ...
    <v-row v-if="show_config_tpv" dense>
      <v-col cols="12">
        La carpeta seleccionada en el anterior apartado, "Directorio de
        importación", determina donde se deben poner los ficheros a importar.
        Estos son product.csv, family.csv y ticket.csv (Productos, familias y
        tíquets respectivamente).<br />
        Al poner los ficheros a importar en dicha carpeta se recopilan sus
        cabeceras y se exponen en el siguiente apartado para que especifiques a
        qué campos corresponden.
      </v-col>
    </v-row>
    ...
    <template v-if="show_config_tpv">
      <template v-for="(domain, domain_name) in model.domain">
        ...
        <v-row :key="domain.title + '-refresh'" dense>
          <v-col cols="12" class="text-center">
            <CtBtn
              :type="stored_config.branding.style.button"
              color="primary"
              @click="headersSync(domain_name)"
            >
        ...
        <v-row :key="domain.title + '-titles'" dense>
        ...
        >
          ...
          <v-col cols="12" lg="4">
            <v-row dense>
              <v-spacer v-if="$vuetify.breakpoint.mdAndDown" />
              <CtSelect
                v-model="domain.fields_columns[field.name]"
                :ct-type="stored_config.branding.style.form"
                class="ma-4"
                :items="domain.columns"
                label="Columna del fichero"
              />
              <v-spacer v-if="$vuetify.breakpoint.mdAndDown" />
            </v-row>
          </v-col>
          <v-col
            cols="12"
            lg="4"
            :class="{ 'body-2': true, 'mt-4': $vuetify.breakpoint.smAndUp }"
          >
            <v-row dense>
              <v-spacer v-if="$vuetify.breakpoint.mdAndDown" />
              {{ field.description }}
...
export default {
  ...
  mounted() {
    ...
    this.get_headers_main_event = (event, domain, headers) => {
      ...
      this.setConfig({
        path: 'import>domain>' + domain + '>columns',
        value: headers
      })
  ...
  beforeDestroy() {
    ...
    console.log('TODO remove event to get headers from api')
  },
