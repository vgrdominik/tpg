<template>
  ...
  <v-row class="ct-card-content-family-list pt-0 pb-0">
          ...
          <v-row>
            ...
            >
              <v-card
                ...
                :elevation="
                  item.id === $store.state.family.current_family ? 5 : 1
                "
                @click="
              ...
              >
                ...
                <v-row dense class="pb-4">
                  <v-spacer />
                  <span
                    class="body-2 text-uppercase primary--text"
    ...
    <v-col
      ...
      md="3"
      lg="2"
      :class="{ 'mt-6': $vuetify.breakpoint.mdAndUp }"
    >
      <v-row dense>
    ...
    familiesPerPageFiltered: {
      get() {
        return this.$vuetify.breakpoint.smAndDown
          ? this.familiesPerPage - 3
          : this.$vuetify.breakpoint.lgAndUp
          ? this.familiesPerPage
          : this.familiesPerPage - 2
      },
      set(newValue) {
        return newValue
      }
      ...
      if (newValue && newValue.length > 0) {
        newValue.forEach(
          (family) =>
            (this.familyImg[
              'family' + family.id
            ] = this.$global_utilities.require_img(
              'family/barRestaurant/' + family.img
            ))
        )
      }
    }
