...
export default {
  ...
  computed: {
    ...
    current_product_to_show_complements: {
      get() {
        if (
          this.current_product_to_show === null ||
          !this.current_product_to_show.complement_ids_available ||
          this.current_product_to_show.complement_ids_available.length === 0
        ) {
          return null
        }

        return this.$store.state.product.products.filter((product) =>
          this.current_product_to_show.complement_ids_available.includes(
            product.id
          )
        )
      },
    ...
    current_product_to_show_complements_groups: {
      get() {
        ...
        const current_product_to_show_complements_groups = []
        this.current_product_to_show_complements.forEach((product) =>
          !current_product_to_show_complements_groups.includes(
            product.complement_taxonomy
          )
            ? current_product_to_show_complements_groups.push(
                product.complement_taxonomy
              )
            : null
        )
    ...
    current_product_to_show_complements_by_group: {
      get() {
        ...
        const productComplementsByGroup = {}
        this.current_product_to_show_complements_groups.forEach((group) => {
          productComplementsByGroup[
            group
          ] = this.current_product_to_show_complements.filter(
            (product) => product.complement_taxonomy === group
          )
        })
    ...
    current_ticket() {
      ...
      return this.$store.state.ticket.tickets.filter(
        (ticket) => ticket.id === this.$store.state.ticket.current_ticket
      )[0]
    },
    ...
    current_ticket_line() {
      ...
      if (
        ticketLineOrTicketLineComplement &&
        ticketLineOrTicketLineComplement.length === 1
      ) {
        // It's a product
        return this.current_ticket.lines.filter(
          (ticketLine) =>
            ticketLine.id_ticket_line === this.currentTicketLineToQuantityModify
        )[0]
      } else {
        // It's a complement
        ...
        if (current_ticket_line) {
          return current_ticket_line.ticket_complements.filter(
    ...
    current_customer: {
      get() {
        ...
        const current_customer = this.$store.state.customer.customers.filter(
          (customer) => customer.id === this.current_ticket.id_customer
        )
  ...
  methods: {
    ...
    addProductToTicket(
      ...
      if (!current_customer) {
        current_customer = this.$store.state.customer.customers.filter(
          (customer) =>
            ...
            (!customerId && (customer.id === 1 || customer.id === '1'))
        )[0]
      }
      if (!current_customer) {
        return false
      ...
      this.$nextTick(() => {
        ...
        if (this.current_ticket.lines[this.current_ticket.lines.length - 1]) {
          currentTicketLineId =
            parseInt(
              this.current_ticket.lines[this.current_ticket.lines.length - 1]
                .id_ticket_line
            ) + 1
        }
        ...
        if (complementsSelection) {
          Object.entries(complementsSelection).forEach(
            (complementSelection) => {
              // complementSelection[0] -> Complement group
              // complementSelection[1] -> Complement selection index
              if (
                complementSelection[1] === null ||
                !this.current_ticket ||
                !this.current_ticket.lines
              ) {
                return
              }
              const productComplement = this
                .current_product_to_show_complements_by_group[
                complementSelection[0]
              ][complementSelection[1]]
              const currentTicketLine = this.current_ticket.lines[
                this.current_ticket.lines.length - 1
              ]
              ...
              if (
                currentTicketLine.ticket_complements[
                  currentTicketLine.ticket_complements.length - 1
                ]
              ) {
                currentTicketLineComplementId =
              ...
              currentTicketLine.ticket_complements.push({
                ...
                price: productComplement.complement_price
                  ? productComplement.complement_price
                  : productComplement.total,
    ...
    payDirectTicket() {
      ...
      if (this.current_ticket.receipt[this.current_ticket.receipt.length - 1]) {
        currentTicketReceiptId =
          parseInt(
            this.current_ticket.receipt[this.current_ticket.receipt.length - 1]
              .id
          ) + 1
      }
